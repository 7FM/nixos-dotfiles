...
let
  ...
  enableSystemdWaybar = swayEnabled;
    ...
    lib.mkIf enable {
      ...
      programs.waybar = {
        ...
        systemd = {
          ...
          # graphical-session.target is started by sway (via HM's
          # systemd.enable wiring), so waybar autostarts in the session.
        ...
        settings = [
          {
            ...
            modules = {
              ...
              "custom/scratchpad-indicator" = {
                ...
                "return-type" = "json";
                "exec" =
                  "${pkgs.sway}/bin/swaymsg -t get_tree | ${pkgs.jq}/bin/jq --unbuffered --compact-output '( select(.name == \"root\") | .nodes[] | select(.name == \"__i3\") | .nodes[] | select(.name == \"__i3_scratch\") | .focus) as $scratch_ids | [..  | (.nodes? + .floating_nodes?) // empty | .[] | select(.id |IN($scratch_ids[]))] as $scratch_nodes | { text: \"\\($scratch_nodes | length)\", tooltip: $scratch_nodes | map(\"\\(.app_id // .window_properties.class) (\\(.id)): \\(.name)\") | join(\"\\n\") }'";
                # "format" = "􏠜 {}";
                "format" = "{}";
      ...
      xdg.configFile."waybar/scripts/custom_gpu.sh" = {
        ...
        + ''echo '{"text": "''
        + (
          lib.optionalString (gpuCfg.mhzFreqCmd != null) "'$clock'GHz"
          + lib.optionalString (gpuCfg.tempCmd != null || gpuCfg.usageCmd != null) " "
        )
        + (
          lib.optionalString (gpuCfg.tempCmd != null) " '$temperature'°C"
          + lib.optionalString (gpuCfg.usageCmd != null) " "
        )
