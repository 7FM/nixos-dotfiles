...
lib.mkMerge [
  ...
  }
  {
    ...
    services.sanoid = {
      ...
      datasets =
        let
          ...
          datasetNames = builtins.attrValues (
            lib.attrsets.mapAttrs' (_: fs: lib.nameValuePair fs.device fs.device) zfsDatasets
          );
        in
        lib.genAttrs datasetNames (_: {
    ...
    systemd.services."acme-${letsEncryptHost}" =
      let
      ...
      in
      {
        ...
        serviceConfig = {
          ...
          ExecStartPre =
            "+"
            + (pkgs.writeShellScript "acme-${letsEncryptHost}-pre-start" ''
          ...
          ExecStopPost =
            "+"
            + (pkgs.writeShellScript "acme-${letsEncryptHost}-post-stop" ''
        ...
        update_router_forwards =
          let
            ...
            createForwardRule =
              ...
              builtins.map (
                p:
                "    IPv6FilterRule(dir=RuleDir.incoming, dst_addr=newIPv6Addr, dst_sport=${toString p}, dst_eport=${toString p}, protocol=IPv6FilterRuleProto.${prot})"
              ) ports;
            forwardRules = builtins.concatStringsSep ",\n" (
              (createForwardRule "tcp" exposedTCPPorts) ++ (createForwardRule "udp" exposedUDPPorts)
            );
          in
          pkgs.writeText "update_router_forwards.py" ''
            from compal import *
            import sys
