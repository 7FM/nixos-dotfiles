...
rec {
  ...
  getSecret = basePath: offsetPath: import (getSecretPath basePath offsetPath);

  # Network port tools
  definePortTCP = port: desc: {
    inherit port desc;
    tcp = true;
  };
  definePortUDP = port: desc: {
    inherit port desc;
    udp = true;
  };
  definePortTCPnUDP = port: desc: (definePortTCP port desc) // (definePortUDP port desc);
  defineInternetExposedPortTCP = port: desc: {
    inherit port desc;
    tcp = true;
    expose_to_internet = true;
  };
  defineInternetExposedPortUDP = port: desc: {
    inherit port desc;
    udp = true;
    expose_to_internet = true;
  };
  defineInternetExposedPortTCPnUDP =
    port: desc: (defineInternetExposedPortTCP port desc) // (defineInternetExposedPortUDP port desc);
  defineHiddenPortTCP = port: desc: {
    inherit port desc;
    tcp = true;
    hidden = true;
  };
  defineHiddenPortUDP = port: desc: {
    inherit port desc;
    udp = true;
    hidden = true;
  };
  defineHiddenPortTCPnUDP =
    port: desc: (defineHiddenPortTCP port desc) // (defineHiddenPortUDP port desc);
  ...
  getAllLocallyExposedXPorts =
    ...
    let
      ...
      uniquePorts =
        assert portSize == uniquePortSize;
        builtins.attrValues (
          builtins.removeAttrs (builtins.mapAttrs (name: value: (builtins.elemAt value 0)) (
