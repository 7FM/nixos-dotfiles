...
Serves ?action=stream (multipart MJPEG) and ?action=snapshot (single JPEG).
...
def acquire_capture():
    global capture_refcount, capture_proc, current_frame
    with new_frame:
        capture_refcount += 1
...
def release_capture():
    global capture_refcount, capture_proc
    proc_to_kill = None
    with new_frame:
        capture_refcount -= 1
